...
export default function ActiveOrdersPage() {
    ...
    const { data: orderData = { data: [], total: 0, pages: 0 }, isLoading } = useQuery({
        queryKey: ["admin-orders-active", page],
        queryFn: async () => {
            const { data } = await api.get(`/admin/orders?status=ACTIVE&page=${page}`);
            return data;
        },
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-border pb-6">
                <div>
                    <h1 className="text-3xl font-heading font-black">Active Orders</h1>
                    <p className="text-muted-foreground text-sm font-bold mt-1">Manage orders currently in progress</p>
                </div>
                <div className="flex items-center gap-2 w-full sm:w-auto">
                    <div className="relative flex-1 sm:w-64">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={18} />
                        <input type="text" placeholder="Search active orders..." className="w-full bg-white border border-border rounded-xl pl-10 pr-4 py-2 text-sm outline-none focus:ring-2 ring-primary" />
                    </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="text-sm font-medium">
                            {orders.length === 0 ? (
                                <tr>
                                    <td colSpan="6" className="p-8 text-center text-muted-foreground">No active orders found.</td>
                                </tr>
                            ) : orders.map((order) => (
                                <tr
                                    key={order.id}
                                ...
                                >
                                    ...
                                    <td className="p-4">
                                        <span className={`px-3 py-1 rounded-full text-xs font-bold ${order.status === 'ON_DELIVERY' ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700'
                                            }`}>
